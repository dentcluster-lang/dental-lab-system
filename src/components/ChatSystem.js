...
import {
    ...
    Mic, X, Download, Trash2, MoreVertical, Check, CheckCheck, FileText,
    Eye, Inbox
} from 'lucide-react';
...
function ChatSystem() {
    ...
    const [orderTab, setOrderTab] = useState('sent'); // 'sent' | 'received'
    ...
    const handleSendOrder = async () => {
        if (!selectedOrder || !selectedRoom) return;
        ...
        try {
            ...
            // 의뢰서 정보를 메시지로 전송
            await addDoc(collection(db, 'chatRooms', roomToUse.id, 'messages'), {
                text: `📋 의뢰서 #${selectedOrder.orderNumber}\n환자명: ${selectedOrder.patientName || '정보 없음'}`,
                senderId: targetId,
                senderName: userData.businessName || userData.companyName || userData.displayName || '익명',
                orderData: {
            ...
            // 채팅방 최근 메시지 업데이트
            await updateDoc(doc(db, 'chatRooms', roomToUse.id), {
                lastMessage: `의뢰서 #${selectedOrder.orderNumber}`,
                lastMessageTime: serverTimestamp()
            });
            ...
            console.error('의뢰서 전송 실패:', error);
            alert('의뢰서 전송에 실패했습니다.');
        }
    };
                                ...
                                >
                                    ...
                                    <div className="message-content">
                                        ...
                                        <div className="message-bubble">
                                            ...
                                            {/* 📋 의뢰서 메시지 표시 */}
                                            {message.messageType === 'order' && message.orderData && (
                                                <div className="order-card">
                                                    ...
                                                    <div className="order-card-body">
                                                        <div className="order-card-row">
                                                            <strong>주문번호:</strong>
                                                            <span>#{message.orderData.orderNumber}</span>
                                                        </div>
                                                        <div className="order-card-row">
                                                            <strong>환자명:</strong>
                                                            <span>{message.orderData.patientName || '정보 없음'}</span>
                                                        </div>
                                                        {message.orderData.deliveryDate && (
                                                            <div className="order-card-row">
                                                                <strong>납기일:</strong>
                                                                <span>
                                                                    {message.orderData.deliveryDate.toDate ?
                                                                     message.orderData.deliveryDate.toDate().toLocaleDateString('ko-KR') :
                                                                     message.orderData.deliveryDate}
                                                                </span>
                                                            </div>
                                                        )}
                                                        {message.orderData.status && (
                                                            <div className="order-card-row">
                                                                <strong>상태:</strong>
                                                                <span className={`order-status ${message.orderData.status}`}>
                                                                    {message.orderData.status === 'completed' ? '완료' :
                                                                     message.orderData.status === 'inProgress' ? '진행중' :
                                                                     message.orderData.status === 'pending' ? '대기' : message.orderData.status}
                                                                </span>
                                                            </div>
                                                        )}
                                                    </div>
                                                    <button
                                                        ...
                                                        onClick={() => navigate(`/view-order/${message.orderData.orderId}`)}
                                                    >
                                                        <Eye size={16} />
                                                        의뢰서 상세보기
                                                    </button>
                    ...
                    <div className="modal-content order-modal" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="modal-body">
                            ...
                            <div className="order-tabs">
                                <button
                                    className={`order-tab ${orderTab === 'sent' ? 'active' : ''}`}
                                ...
                                <button
                                    className={`order-tab ${orderTab === 'received' ? 'active' : ''}`}
                            ...
                            <div className="order-list">
                                {(() => {
                                    const orders = orderTab === 'sent' ? sentOrders : receivedOrders;
                                            ...
                                            <div className="order-item-header">
                                                ...
                                                <span className={`order-status ${order.status || 'pending'}`}>
                                                    {order.status === 'completed' ? '완료' :
                                                     order.status === 'inProgress' ? '진행중' :
                                                     order.status === 'pending' ? '대기' : order.status}
                                                </span>
                                            </div>
                                            <div className="order-item-content">
                                                <p className="patient-name">
                                                    <strong>환자명:</strong> {order.patientName || '정보 없음'}
                                                </p>
                                                {order.deliveryDate && (
                                                    <p className="delivery-date">
                                                        <strong>납기일:</strong> {
                                                            order.deliveryDate.toDate ?
                                                            order.deliveryDate.toDate().toLocaleDateString('ko-KR') :
                                                            order.deliveryDate
                                                        }
                                                    </p>
                                                )}
