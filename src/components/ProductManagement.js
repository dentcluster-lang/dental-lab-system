...
import { collection, query, where, getDocs, updateDoc, deleteDoc, doc, writeBatch, addDoc, Timestamp } from 'firebase/firestore';
import { ref, uploadBytes, getDownloadURL } from 'firebase/storage';
import { storage } from '../firebase/config';
import * as XLSX from 'xlsx';
import { 
    Package, Edit2, Trash2, Eye, EyeOff, Search, TrendingUp, 
    Image as ImageIcon, ChevronLeft, ChevronRight, Download, 
    FileSpreadsheet, Images, Copy, Upload, Plus, CheckCircle, XCircle,
    Filter, RefreshCw
} from 'lucide-react';

const ProductManagement = ({ userInfo }) => {
    ...
    const sortOptions = [
        ...
        { value: 'orderCount-desc', label: '판매량 높은순' },
    ...
    const loadProducts = useCallback(async () => {
        try {
            ...
            console.log(`✅ ${productsData.length}개 상품 로드 완료`);
    ...
    const downloadExcelTemplate = () => {
        ...
        const categorySheet = XLSX.utils.json_to_sheet([
            { '카테고리 값': 'impression', '설명': '인상재' },
            { '카테고리 값': 'crown', '설명': '크라운/브릿지' },
            { '카테고리 값': 'denture', '설명': '의치' },
            { '카테고리 값': 'implant', '설명': '임플란트' },
            { '카테고리 값': 'orthodontic', '설명': '교정' },
            { '카테고리 값': 'ceramic', '설명': '세라믹' },
            { '카테고리 값': 'metal', '설명': '금속재료' },
            { '카테고리 값': 'equipment', '설명': '장비/기구' },
            { '카테고리 값': 'consumable', '설명': '소모품' },
            { '카테고리 값': 'other', '설명': '기타' }
        ]);
        XLSX.utils.book_append_sheet(wb, categorySheet, '카테고리 목록');

        const guideSheet = XLSX.utils.json_to_sheet([
            { '단계': '1', '설명': '이미지 일괄 업로드 버튼을 클릭하여 상품 이미지들을 업로드하세요' },
            { '단계': '2', '설명': '업로드된 이미지 URL을 복사하여 이미지URL 컬럼에 붙여넣으세요' },
            { '단계': '3', '설명': '상품 정보를 입력하세요 (필수: 상품명, 카테고리, 판매가)' },
            { '단계': '4', '설명': '카테고리는 카테고리 목록 시트를 참고하여 정확히 입력하세요' },
            { '단계': '5', '설명': '완성된 엑셀 파일을 "엑셀로 일괄 등록" 버튼으로 업로드하세요' }
        ]);
        XLSX.utils.book_append_sheet(wb, guideSheet, '사용안내');
    ...
    const filteredProducts = products.filter(product => {
        ...
        } else if (field === 'price' || field === 'viewCount' || field === 'orderCount') {
    ...
    if (loading) {
        return (
            <div style={styles.loadingContainer}>
                ...
                <div style={styles.loadingText}>상품 목록을 불러오는 중...</div>
            </div>
        );
    }
                        ...
                        <table style={styles.table}>
                            <thead>
                                <tr style={styles.tableHeader}>
                                    ...
                                    <th style={{...styles.th, width: '200px'}}>작업</th>
                                </tr>
                            </thead>
                            <tbody>
                                            ...
                                            <td style={styles.td}>
                                                <div style={styles.actionButtons}>
                                                    <button
                                                        ...
                                                        onClick={() => window.location.href = `/product-edit/${product.id}`}
                                                        title="수정"
                                                    >
                                                        <Edit2 size={14} />
                                                    </button>
