...
function ChatSystem() {
    ...
    const handleOrderSelect = async (order) => {
        ...
        if (selectedRoom.isVirtual) {
            try {
                ...
                const chatRoomRef = await addDoc(collection(db, 'chatRooms'), {
                    participants: selectedRoom.participants,
                    participantNames: selectedRoom.participantNames,
                    lastMessage: `[의뢰서] ${order.orderNumber}`,
                    lastMessageTime: serverTimestamp(),
                    createdAt: serverTimestamp()
                });
        ...
        try {
            ...
            console.log('의뢰서 전송 완료:', order.orderNumber);
        } catch (error) {
    ...
    return (
        <div className="chat-system">
                                ...
                                >
                                    ...
                                    <div className="message-content">
                                        ...
                                        <div className="message-bubble">
                                            {message.text && !message.orderData && <p>{message.text}</p>}
                                            ...
                                            {message.orderData && (
                                                <div className="order-message-card">
                                                    <div className="order-card-header">
                                                        <FileText size={16} />
                                                        <span className="order-card-title">의뢰서</span>
                                                    </div>
                                                    <div className="order-card-body">
                                                        <div className="order-card-row">
                                                            <span className="label">주문번호:</span>
                                                            <span className="value">{message.orderData.orderNumber}</span>
                                                        </div>
                                                        <div className="order-card-row">
                                                            <span className="label">환자명:</span>
                                                            <span className="value">{message.orderData.patientName}</span>
                                                        </div>
                                                        {message.orderData.items && message.orderData.items.length > 0 && (
                                                            <div className="order-card-items">
                                                                {message.orderData.items.slice(0, 2).map((item, idx) => (
                                                                    <span key={idx} className="order-item-tag">
                                                                        {item.toothNumber} - {item.prosthesisType || item.workType}
                                                                    </span>
                                                                ))}
                                                                {message.orderData.items.length > 2 && (
                                                                    <span className="order-item-tag more">
                                                                        +{message.orderData.items.length - 2}개
                                                                    </span>
                                                                )}
                                                            </div>
                                                        )}
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={() => navigate(`/order/${message.orderData.id}`)}
                                                    >
                                                        의뢰서 보기
                                                    </button>
                                                </div>
            ...
            <OrderSelectionModal
                ...
                userId={userData?.companyId || currentUser?.uid}
            />
        </div>
    );
}
